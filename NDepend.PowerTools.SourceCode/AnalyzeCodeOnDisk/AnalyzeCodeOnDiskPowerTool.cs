   ...
   class AnalyzeCodeOnDiskPowerTool : IPowerTool {
      ...
      public void Run() {
         ...
         using (new ConsoleColorPreserver(ColorSet.Error)) {
            Console.WriteLine(@"This Power Tool relies on some NDepend API in IVisualStudioManager that doesn't work with .NET Core");
            return;
         }
#else
         ...
         foreach (var vsVersion in new[] {
            VisualStudioVersion.V10_2010,
            VisualStudioVersion.V12_2013,
            VisualStudioVersion.V14_2015,
            VisualStudioVersion.V15_2017,
            VisualStudioVersion.V16_2019,
         }) {
            if (!visualStudioManager.IsVisualStudioVersionInstalled(vsVersion)) { continue; }

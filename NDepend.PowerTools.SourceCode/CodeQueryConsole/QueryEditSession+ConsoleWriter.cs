      ...
      partial class ConsoleWriter {
         ...
         internal void ShowQueryCompiledErrors(State state, IQueryCompiledError queryCompiledError) {
            ...
            foreach (var error in queryCompiledError.Errors) {
               var location = new Location(lines, error.SubStringStartPos);
               // Below use a padding string to align error after location
               Console.WriteLine($"Ln:{location.X}{(location.X < 10 ? " ":"")} Col:{location.Y}{(location.Y < 10 ? " " : "")}  {error.Description}");
            }

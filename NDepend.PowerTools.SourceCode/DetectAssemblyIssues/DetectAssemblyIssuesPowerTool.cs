...
namespace NDepend.PowerTools.DetectAssemblyIssues {
   class DetectAssemblyIssuesPowerTool : IPowerTool {
      ...
      public string[] Description {
         get { 
            return new [] {
               "Check several potential deployment issues,", 
      ...
      public void Run() {
         ...
         IProject project;
         if (!ProjectAnalysisUtils.TryChooseOrBuildProject(out project)) { return; }
         ...
         //--------------------------------------------------------------------------------------------
         ShowConsoleNewCheck(new[] { "Check that all application assemblies share ", "a common target framework name (net5.0, netstandard2.1, net472...)" });
         CoherencyChecker.Go(applicationAssembliesInfos,
            assemblyInfo => assemblyInfo.TargetFrameworkName,

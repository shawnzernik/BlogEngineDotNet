...
namespace NDepend.PowerTools.SharedUtils {
   public abstract class CodeElementsDisplayerBase<TDisplayInfo> where TDisplayInfo : class {
      ...
      internal void Display() {
         ...
         try {
            while (true) {
               ...
               // 13Oct2021: Console.CursorTop is not updated on Linux terminal
               if (nbLines <= 0 && !OSHelper.IsOnWindows) {
                  nbLines = 2;
                  Console.CursorTop += nbLines;
               }
               Debug.Assert(nbLines > 0, "If this assert is broken, it is a symptom that the console height buffer maximum has been reached. PowerTools needs to be restarted.");
      ...
      ASK_MENU_UP_DOWN_ENTER:
               ...
               // 13Oct2021: since Console.CursorTop is not updated on Linux terminal cannot navigate between result
               if (!OSHelper.IsOnWindows) { showMenu = false;}
               var choice = ConsoleAskUpDownEnterAllNextQuit(showMenu, showEnd, cursorTopMenu);
               if (!OSHelper.IsOnWindows) { choice = UpDownEnterAllNextQuit.Quit; }
               switch (choice) {
